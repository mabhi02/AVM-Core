...
from tqdm.auto import tqdm  # Import missing tqdm
...
def parse_args():
    ...
    parser.add_argument('--chunk-size', type=int, default=256,  # Reduced from 512 to 256
                       help='Size of chunks for memory efficient processing')
    parser.add_argument('--max-len', type=int, default=256,  # Added default of 256
...
def update_configs(model_config: dict, train_config: dict, args) -> tuple:
    ...
    if args.max_len is not None:
        ...
        # Always set max_len to 256 if not provided
        if 'model' not in model_config:
            model_config['model'] = {}
        model_config['model']['max_len'] = 256
        logger.info(f"Setting max_len to default: 256")
...
def validate_config(config: dict) -> dict:
    ...
    # Ensure max_len is set properly in model config
    if 'model' in config:
        if 'max_len' not in config['model'] or config['model']['max_len'] > 256:
...
def train(model: AVMCore, train_config: dict, train_dataset: ProofDataset, 
    ...
    # Ensure sensible batch size for CPU
    if device.type == 'cpu' and train_config['training']['batch_size'] > 4:
        train_config['training']['batch_size'] = 4
        logger.info(f"Reduced batch size to 4 for CPU training")
    
    # Create dataloaders with appropriate num_workers
    num_workers = 0 if device.type == 'cpu' else 2  # No workers for CPU to avoid overhead
...
def evaluate(model: AVMCore, val_dataset: ProofDataset, device: torch.device) -> float:
    ...
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset,
        batch_size=4,  # Smaller batch size for evaluation, especially on CPU
        shuffle=False,
        collate_fn=collate_proofs
    )
...
def prepare_model_config(config: dict) -> dict:
    ...
    model_params = {
        ...
        'max_len': config['model'].get('max_len', 256),  # Default to 256
    ...
    try:
        ...
        if args.mode == 'train':
            ...
            train_dataset = ProofDataset(
                ...
                max_len=model_params['max_len']  # Pass max_len to dataset
            )
            ...
            # Save just the model weights to model.pth
